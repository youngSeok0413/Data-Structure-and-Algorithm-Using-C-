...
public:
	...
	std::vector<int>::iterator lookup(int key) {
		...
		if (data1[hash_value1] == key) {
			std::cout << "1 번 테이블에서 " << key << "을(를) 발견하였습니다." << std::endl;
			return data1.begin() + hash_value1;
		}

		auto hash_value2 = hash2(key);
		if (data1[hash_value2] == key) {
			std::cout << "2 번 테이블에서 " << key << "을(를) 발견하였습니다." << std::endl;
	...
	void erase(int key) {
		...
		if (position != data2.end()) {
			*position = -1;
			std::cout << key << "에 해당하는 원소를 삭제하였습니다." << std::endl;
		}
		else {
			std::cout << key << "를 찾지 못하였습니다." << std::endl;
		}
	}
